...
class Encoder(snt.AbstractModule):
    def __init__(self, name="Encoder", edge_one_hot=False, edge_vocab=None):
        ...
        with self._enter_variable_scope():
            self._encode = graph_net_modules.GraphIndependent(edge_model_fn=edge_model,
    ...
    def one_hot_edge_model_fn(self):
        """
        Function for the edge one hot encoding
        :return: the encoded edge
        """
        return self.edge_vocab
...
class SimpleGraphAttention(snt.AbstractModule):
...
class GraphAttention(snt.AbstractModule):
    def __init__(self, edge_output_size=None, node_output_size=None, global_output_size=None,
        ...
        self._network = graph_net_modules.GraphNetwork(edge_model_fn=self.edge_model_fn,
                                                       ...
                                                       global_model_fn=self.global_model_fn,
                                                       reducer=tf.unsorted_segment_sum)
        self._attention = sonnet_nets.GraphAttentionLayer(input_size=1024, output_size=2, heads=5)
    ...
    def _build(self, input_op, num_processing_steps):
        ...
        for _ in range(num_processing_steps):
            core_input = utils_tf.concat([latent0, latent], axis=1)
            latent = self._network(core_input)
            attended = self._attention(latent0)
            latent = latent.replace(nodes=tf.stack([latent.nodes, attended.nodes], axis=1))
            decoded_op = self._output_transform(latent)
            output_ops.append(decoded_op)
        return output_ops
